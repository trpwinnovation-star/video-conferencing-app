...
export function VideoTile({ trackRef, isPinned = false }: VideoTileProps) {
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-stone-900 transition-all duration-500 h-full w-full rounded-xl sm:rounded-2xl group",
        isSpeaking && !isScreenShare ? "ring-2 ring-[#c16d18] ring-offset-2 ring-offset-stone-950" : "",
    ...
    >
      ...
      {hasVideo && !isVideoMuted ? (
        ...
        <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-stone-900">
          {/* Subtle Background Glow */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#c16d18]/5 via-transparent to-[#c16d18]/5" />

          <div className="relative w-14 h-14 sm:w-20 sm:h-20 md:w-22 md:h-22 bg-gradient-to-br from-[#c16d18] to-[#965310] rounded-full flex items-center justify-center text-xl sm:text-2xl md:text-3xl font-extrabold shadow-lg shadow-[#c16d18]/20 border border-white/10 text-white animate-in zoom-in duration-500">
      ...
      <div className="absolute bottom-2 left-2 right-2 sm:bottom-3 sm:left-3 sm:right-3 flex items-center justify-between pointer-events-none z-10">
        <div className="bg-black/60 backdrop-blur-md px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl flex items-center gap-1 sm:gap-1.5 shadow-md text-white transition-transform duration-300 pointer-events-auto">
          {isScreenShare && <Monitor size={11} className="sm:w-3.5 sm:h-3.5 text-[#c16d18]" />}
          <ParticipantName
            participant={participant}
            className="text-white text-[10px] sm:text-[11px] md:text-[12px] font-bold truncate max-w-[60px] sm:max-w-[80px] md:max-w-none"
          />
          {participant.isLocal && <span className="text-[8px] sm:text-[9px] md:text-[10px] text-[#e8943a] font-black ml-0.5 uppercase tracking-wider">(You)</span>}
          {isScreenShare && <span className="text-[8px] sm:text-[9px] md:text-[10px] text-stone-300 font-bold ml-0.5 uppercase tracking-wider">Screen</span>}
          {isPinnedParticipant && !isScreenShare && <Pin size={10} className="sm:w-3 sm:h-3 text-[#c16d18] ml-0.5" />}
        </div>

        {!isScreenShare && (
          <div className={cn(
            "p-1.5 sm:p-2 rounded-lg sm:rounded-xl flex items-center justify-center backdrop-blur-md shadow-md transition-all duration-300 pointer-events-auto",
            isAudioMuted
